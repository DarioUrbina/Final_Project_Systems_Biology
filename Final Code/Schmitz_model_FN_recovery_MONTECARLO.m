function dydt = Schmitz_model_FN_recovery_MONTECARLO(t,y,params)
...
%HKflux
Vmax16 = params(1,17);     %VHKmax  mM/s 
KATP16 = params(2,17);      %KATP
KGlc16 = params(3,17);        %KGlc
KATPGlc = params(4,17);        %KATPGlc 
KG6P16 = params(5,17);      %KG6P
KGlcG6P = params(6,17);     %KGlcG6P
Glc = params(7,17);      %Glc 
...
% fraca = 0.4;
% fracb = 0.6;
fluxGP = params(9,1); %mM/min [0 0.75e-3]     GLY production flux           COMING FROM MONTE CARLO                                           MONTE CARLO
...
%HKflux
HKflux = (Vmax16*ATP*Glc/(KATP16*KATPGlc))/(1+ATP/KATP16+Glc/KGlc16+ATP*Glc/(KATP16*KATPGlc)+G6P/KG6P16+Glc*G6P/KGlc16*KGlcG6P);

%ATP Reactions:ATPase
VATPase = params(8,17); % unit:mM/s  [0.0001 TO 0.01]                             MONTE CARLO
output = 0.01*LAC; %unit:mM/s
...
%% define ODEs
dydt(1,1)= fluxGP; %equation for GLY production flux
% dydt(1,1)= -fluxGP; %equation for GLY
dydt(2,1)= -fluxGP-VPGLM; %equation for G1P
% dydt(2,1)= fluxGP-VPGLM; %equation for G1P
dydt(3,1)= VPGLM+HKflux-VPGI; %equation for G6P
